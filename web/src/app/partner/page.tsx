        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            <div className="flex items-center gap-2">
              <span className="text-xs font-black uppercase px-2 py-0.5 rounded bg-emerald-50 dark:bg-emerald-950/50 text-emerald-800 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800">
                Partner Cabinet
              </span>
              <span className="text-xs text-slate-500 dark:text-slate-400">Do‘kon & Hamkor Boshqaruvi</span>
            </div>
          ...
          <div className="flex items-center gap-3">
            <div className="text-right">
              <div className="text-xs text-slate-400 dark:text-slate-500">Jami qurilmalar:</div>
              <div className="text-xl font-black text-slate-900 dark:text-white">{devices.length} ta</div>
            </div>
          </div>
        </div>
            ...
            <div className="space-y-6 lg:col-span-1">
              ...
              <div className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm">
                <h3 className="font-extrabold text-base text-slate-900 dark:text-white mb-1 flex items-center gap-2">
                  <Plus className="w-4 h-4 text-emerald-600 dark:text-emerald-400" /> Bitta IMEI qo‘shish
                </h3>
                ...
                <form onSubmit={handleAddSingle} className="space-y-3">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full text-sm font-mono px-3.5 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:border-brand-600 dark:focus:border-brand-400 outline-none"
                    />
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">Holati:</label>
                    <select
                      value={singleStatus}
                      onChange={(e: any) => setSingleStatus(e.target.value)}
                      className="w-full text-xs font-semibold px-3.5 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:border-brand-600 outline-none"
                    >
                  ...
                  <button
                    ...
                    className="w-full py-2.5 px-4 rounded-xl bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 dark:bg-emerald-500 dark:hover:bg-emerald-600 disabled:opacity-50 text-white font-bold text-xs shadow-md transition flex items-center justify-center gap-1.5"
                  >
                    {singleLoading ? (
              ...
              <div className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm">
                <div className="flex items-center justify-between mb-1">
                  <h3 className="font-extrabold text-base text-slate-900 dark:text-white flex items-center gap-2">
                    <FileSpreadsheet className="w-4 h-4 text-brand-600 dark:text-brand-400" /> CSV / Excel Import
                  </h3>
                  <button
                    type="button"
                  ...
                  >
                    <Sparkles className="w-3 hand-3" /> Namuna
                  </button>
                </div>
                <p className="text-xs text-slate-500 dark:text-slate-400 mb-4">
                  Format: <code className="bg-slate-100 dark:bg-slate-800 px-1 py-0.5 rounded text-slate-700 dark:text-slate-300">imei,status</code>
                </p>
                ...
                <textarea
                  ...
                  className="w-full text-xs font-mono p-3 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:border-brand-600 outline-none mb-3"
                />

                {importReport && (
                  <div className="mb-3 p-3.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 text-xs space-y-1.5">
                    <div className="flex items-center justify-between font-bold">
                      <span className="text-emerald-700 dark:text-emerald-300">Muvaffaqiyatli: {importReport.successCount} ta</span>
                      <span className="text-rose-700 dark:text-rose-300">Xato: {importReport.failCount} ta</span>
                    </div>
                    {importReport.errors?.length > 0 && (
                      <div className="text-[11px] text-rose-600 dark:text-rose-400 pt-1 border-t border-slate-200 dark:border-slate-700">
                ...
                <button
                  ...
                  className="w-full py-2.5 px-4 rounded-xl bg-brand-600 hover:bg-brand-700 active:bg-brand-800 dark:bg-brand-500 dark:hover:bg-brand-600 disabled:opacity-50 text-white font-bold text-xs shadow-md transition flex items-center justify-center gap-1.5"
                >
                  {csvLoading ? (
                    <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                  ) : (
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="text-xs font-mono pl-8 pr-4 py-2 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:border-brand-600 outline-none w-full sm:w-64"
                    />
